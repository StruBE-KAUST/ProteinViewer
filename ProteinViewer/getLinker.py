...
def runRanch(timer):
	...
	"""
	global ranch 
	ranch = subprocess.Popen('cd {} && ranch'.format(tempDir), shell=True, stdin=PIPE, stdout=PIPE, preexec_fn=os.setsid)
...
def getLinker(domRanges, allRanges, new, grabNum, sequence, rep, tempDir):	
	...
	pulchraInput = '{} {}/00001eom.pdb'.format(pulchrapath, tempDir)
	# runs pulchra with pdb from ranch
	pulchra = subprocess.Popen(pulchraInput, shell=True)
	pulchra.wait()

	m = B.PDBModel('{}/00001eom.rebuilt.pdb'.format(tempDir))
	if new == True:
		m = m.centered()
	...
	for i in xrange(len(allRanges)):
		...
		if allRanges[i] in domRanges:
			...
			pdb_name = '{}/dom'.format(tempDir) + str(domCount) + '.pdb'
			if new == True:
				dom_name = '{}/pdb'.format(tempDir) + str(domCount) + '.pdb'
				ori_name = '{}/pdb'.format(tempDir) + str(domCount) + 'ori.pdb'
				piece.writePdb(ori_name)
				piece.writePdb(dom_name)
			domCount = domCount + 1
		else:
			# if we're dealing with a linker:
			pdb_name = '{}/link'.format(tempDir) + str(linkCount) + '.' + str(grabNum) + '.pdb'
	...
	if new == True:
		...
		for i in xrange(domCount):
			...
			vmd = subprocess.Popen('cd {}'.format(vmdpath), shell=True, stdin=PIPE)
			vmd.communicate(input=b'\n axes location off \n mol new {}/{} \n mol rep {} \n mol addrep 0 \n mol delrep 0 0 \n scale to 0.05 \n render Wavefront {}/{} \n quit \n'.format(tempDir, pdb_name, rep, tempDir, obj_name))
			vmd.wait()
