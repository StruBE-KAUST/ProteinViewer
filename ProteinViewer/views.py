...
class SubmitPdbFileView(View):
    ...
    # trying to make it all session-specific..?

    def generate_viewer_page(self, request, form):

        rep = 'newcartoon' # TODO: take representation from the form using request.REP / smth
        ...
        request.session['temp'] = tempDir

        # session = request.session._get_or_create_session_key()
        # print 'session key'
        # print session

        # with open(os.path.join(tempDir, 'session.txt'), 'wb+') as destination:
        ...
        # creating non-temporary files so that we can access them later..? And 
        # especially for the naming because we need to know the order and the names..
        # (unless we keep making arrays with all the file names.. hmm.)
        ...
        pieces = getLinker(domRanges, allRanges, True, 0, 'sequence.fasta', rep, tempDir) #TODO: take representation as well
        ...
        for i in xrange(domains):
            ...
            assets = '<a-asset-item id="dom_model' + str(i) + '\" src="/media/%s/' %(temp) + obj_name + '\"></a-asset-item>' + '<a-asset-item id="dom_mat' + str(i) + '" src="/media/%s/' %(temp) + mtl_name + '"></a-asset-item>' 
        ...
        for i in xrange(linkers):
            ...
            assets = '<a-asset-item id="link_model' + str(i) + '\" src="/media/%s/' %(temp) + obj_name + '\"></a-asset-item>' + '<a-asset-item id="link_mat' + str(i) + '" src="/media/%s/' %(temp) + mtl_name + '"></a-asset-item>' 
        ...
        context = {'assets': asset_string, 'entities': entity_string, 'count': pieces, 'ranges': ranges, 'lines': lineCount, 'shift': linkShift, 'rep': rep, 'temp': tempDir}
